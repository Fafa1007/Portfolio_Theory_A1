function portfolioexamples_plot_geometric(varargin)
...
for i = 2:nargin
	...
	if strcmpi(plottype,'line')
		for k = 1:size(plotrsk,2)
			plot(sqrt(12)*plotrsk(:,k), (1 + plotret(:,k)).^12 - 1, plotstyle, 'LineWidth', plotline);
			if i == 2 && k == 1
				hold on
			end
	...
	% scatter plot
		...
		if any(plotstyle == '.')
			scatter(sqrt(12)*plotrsk, (1 + plotret).^12 - 1, plotstyle);
		else
			scatter(sqrt(12)*plotrsk, (1 + plotret).^12 - 1, plotstyle, 'Filled');
		end			
		if i == 2
			hold on
		end
		if ~isempty(plotlabel)
			for k = 1:numel(plotrsk)
				if ~isempty(plotlabel{k})
					text(sqrt(12)*plotrsk(k) + 0.005, (1 + plotret(k)).^12 - 1, plotlabel{k}, 'FontSize', 8);
				end
			end
		end
	end
